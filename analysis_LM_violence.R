...
estimates = foreach(b = 1:B, .combine = "rbind") %dopar% {
  ...
  X0$treatment = 0
  newdata = rbind(X, X1, X0)
  Qinit = SuperLearner(Y=bootData$any_violence, X=X, newX=newdata, SL.library=SL.library, family="binomial")
  QbarAW = Qinit$SL.predict[1:n]
  Qbar1W = Qinit$SL.predict[(n+1): (2*n)] 
  Qbar0W = Qinit$SL.predict[(2*n+1): (3*n)] 
  ...
  logitUpdate = glm(bootData$any_violence ~ -1 +offset(qlogis(QbarAW)) + H.AW, family=binomial) 
  eps = logitUpdate$coef
