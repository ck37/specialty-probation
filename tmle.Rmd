...
# Naive estimate
summary(glm(any_arrest ~ treatment, data=data_arrest, family="binomial"))
summary(glm(any_violence ~ treatment, data=data_violence, family="binomial"))
(table("Tx"=data_arrest$treatment, "Arrest"=data_arrest$any_arrest, useNA="ifany"))

# Save our data prior to analysis.
save(W_violence, W_arrest, data_violence, data_arrest, file="data/tmle-data-prep.RData")
```
...
if (F) {
  # Manual DSA test.
  # Turn off xgb and RF to speed up the test.
  custom_lib = create_SL_lib(num_cols = ncol(W_arrest), detailed_names = T, dsa=T, xgb=F, rf=F)
  sl_lib = custom_lib$lib
  sl_lib
  
  set.seed(1)
  system.time({
  # Takes about 6 minutes on 4 cores without outer cross-validation, or XGB/RF.
  dsa_arrest = estimate_effect(Y=data_arrest$any_arrest,
                      A=data_arrest$treatment, W=W_arrest,
                      parallel = "multicore", sl_lib = sl_lib,
                      cluster=cluster, cv_folds = cv_folds,
                      crossvalidate=F, outer_cv_folds = 10)
  })
  dsa_arrest
}
```
...
wgts = 1/result_arrest2$ghat$SL.predict
summary(wgts)
...
cat("Review distribution of weights (1/Ghat).\n")
wgts = 1/result_violence2$ghat$SL.predict
summary(wgts)
# Review 10 highest weights.
wgts[order(wgts, decreasing=T)][1:10]
